...
export default function ProveedorForm({
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLocalError(null);
    if (!nombreComercial.trim()) { setLocalError('El nombre comercial es obligatorio'); return; }
    if (!tipoContribuyenteId) { setLocalError('El tipo de contribuyente es obligatorio'); return; }
    ...
    <form className="empresa-form" onSubmit={handleSubmit}>
      <div className="form-head">
        <div>
          ...
          <h3>{mode === 'edit' ? 'Editar proveedor' : 'Crear proveedor'}</h3>
        </div>
      ...
      <div className="form-grid">
        ...
        <label>
          ...
          >
            <option value="">Selecciona tipo</option>
            {tipoContribuyenteOptions.map(opt => (
              <option key={opt.value} value={opt.value}>{opt.label}</option>
            ))}
          </select>
